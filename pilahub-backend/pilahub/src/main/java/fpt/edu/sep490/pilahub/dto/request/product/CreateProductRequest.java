...
public record CreateProductRequest(
                @Schema(description = "Category ID", example = "123e4567-e89b-12d3-a456-426614174000", requiredMode = Schema.RequiredMode.REQUIRED) @NotNull(message = "Category ID must not be null") UUID categoryId,

                @Schema(description = "Product name", example = "Professional Pilates Mat", requiredMode = Schema.RequiredMode.REQUIRED) @NotBlank(message = "Product name must not be blank") @Size(max = 255, message = "Product name must not exceed 255 characters") String name,
                ...
                @Schema(description = "Product price", example = "59.99", requiredMode = Schema.RequiredMode.REQUIRED) @NotNull(message = "Price must not be null") @DecimalMin(value = "0.0", message = "Price must not be negative") Double price,
                ...
                @Schema(description = "Product expired date-time (required when categoryType is SUPPLEMENT)", example = "2026-12-31T00:00:00Z") Instant expiredDate,
        ...
        @AssertTrue(message = "regionSupported must not be null or empty when installationSupported is true")
        public boolean isRegionSupportedValidForInstallation() {
                return !Boolean.TRUE.equals(installationSupported)
                                || (regionSupported != null && !regionSupported.isEmpty());
        }

        @AssertTrue(message = "expiredDate must not be null when categoryType is SUPPLEMENT")
        public boolean isExpiredDateValidForSupplement() {
                return categoryType != CategoryType.SUPPLEMENT || expiredDate != null;
        }
}
